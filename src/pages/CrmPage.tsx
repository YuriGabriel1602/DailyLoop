...
import { Instagram, Kanban, MessageCircle, Phone, Plus, Rows3, Users } from "lucide-react";
...
function StageColumn({ stage, contacts }: { stage: (typeof STAGES)[number]; contacts: Contact[] }) {
...
function ListView({ contacts, onStageChange }: { contacts: Contact[]; onStageChange: (id: number, stage: string) => void }) {
  return (
...
export default function CrmPage() {
  ...
  const addContact = async () => {
    ...
    const created = await api.post<Contact>("/api/crm/contacts", {
      name: newName,
      channel: newChannel,
        ...
        {loading ? (
          ...
          <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
            <div className="flex h-full gap-3 overflow-x-auto pb-4">
              {STAGES.map((stage) => (
                <StageColumn key={stage.id} stage={stage} contacts={contacts.filter((c) => c.stage === stage.id)} />
              ))}
            </div>
          </DndContext>
        ) : (
          <ListView contacts={contacts} onStageChange={changeStage} />
        )}
      </div>
    </div>
  );
